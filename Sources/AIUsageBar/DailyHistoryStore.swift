...
enum DailyHistoryStore {
    ...
    private static func load() -> [String: DayEntry] {
    ...
    private static func save(_ store: [String: DayEntry]) {
    ...
    /// being captured once it's already in the past.
    static func record(_ trend: DailyTrend) {
    ...
    static func backfill(_ trend: DailyTrend) -> DailyTrend {
        lock.lock()
        let store = load()
        lock.unlock()
